...
### 近期对话历史（最近 6 条，按时间正序；回复时请避免重复已说过的内容）
...
- **WM 中出现 `[认知警告]` 条目**：说明我的推理结论已多轮重复，可能存在信念固化。本轮**必须**执行一个可产生新证据的实际动作（`file.read` / `shell.run` / `memory.search`），而不是再次重申相同分析。若当前结论仍正确，需要有新证据支撑它
