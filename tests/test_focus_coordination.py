...
def test_run_cycle_skips_auto_tick_when_unfocused_waiting_task_exists():
    asyncio.run(_run_cycle_skips_auto_tick_when_unfocused_waiting_task_exists())


def test_run_cycle_skips_direct_tick_when_unfocused_waiting_task_exists():
    asyncio.run(_run_cycle_skips_direct_tick_when_unfocused_waiting_task_exists())
...
async def _run_cycle_skips_auto_tick_when_unfocused_waiting_task_exists():
    from core.loop.cycle.driver import _run_cycle_impl
    from store.task import TaskStore

    with tempfile.TemporaryDirectory() as d:
        store = TaskStore(Path(d) / "run-cycle-unfocused-waiting-dispatcher.db")
        await store.open()
        try:
            await store.add_task(
                "未绑定焦点的等待任务",
                goal="waiting task 不应触发 auto tick",
                status="waiting",
                wait_kind="external",
            )
...
async def _run_cycle_skips_direct_tick_when_unfocused_waiting_task_exists():
    from core.loop.cycle.driver import _run_cycle_impl
    from store.task import TaskStore

    with tempfile.TemporaryDirectory() as d:
        store = TaskStore(Path(d) / "run-cycle-unfocused-waiting-direct.db")
        await store.open()
        try:
            await store.add_task(
                "未绑定焦点的等待任务",
                goal="waiting task 不应触发 direct auto tick",
                status="waiting",
                wait_kind="external",
            )
