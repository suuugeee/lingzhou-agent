...
```jsonc
{
  ...
  "vision_model": "copilot/gpt-5.4",      // 识图/视觉模型
  "routing": {
...
| 键 | 默认 | 说明 |
|----|------|------|
| `vision_model` | "copilot/gpt-5.4" | `image.analyze` 优先使用的识图/视觉模型；设为 `null` 时按模型目录自动选择可识图模型 |
...
| `memory.wm_token_budget_ratio` | 0.20 | 工作记忆 token 预算占 judgment 输入预算比例 |
| `memory.max_events` | 500 | 情节记忆最大事件数 |
| `memory.semantic_decay_lambda` | 0.1 | 语义记忆衰减率 |
| `memory.embedding_provider` | "local" | 独立 embedding provider；`local`=本地模型，`none`=禁用向量函数，其他值=使用同名 provider |
| `memory.embedding_model` | null | embedding 模型 ID；与 `embedding_provider` 组合后可独立于主聊天模型 |
| `memory.embedding_fallback` | "fts" | embedding 不可用时保留 FTS5/关键词检索 |
